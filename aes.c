...
/*
 * 1.forward s-box 加密：使用sbox(替换表)字节代替输入状态的字节
 */
...
void aes_encrypt(u8* in, u8* expkey, u8* out) {
    ...
    // 先进行AES_ROUND-1轮的加密，再进行一轮S盒和行变换
...
void aes_apply_cbc_chain_data(u8* cbc_chain_data, u8* src, u8* dst) {
    ...
    for (i = 0; i < AES_KEY_LENGTH; i++) { //AES_KEY_LENGTH长度为16 
...
void aes_cbc_encrypt_blocks(u8* key_exp, u8* src, u8* dst, u32 num_aes_blocks) {
    u8* zero_key = (u8*)"0123456789abcde"; // 初始矩阵 128位加密， 长度必须是16字节
...
void aes_cbc_decrypt_blocks(u8* key_exp, u8* src, u8* dst, u32 num_aes_blocks) {
    ...
    u8* zero_key = (u8*)"0123456789abcde";  //初始矩阵 128位加密， 长度必须是16字节
    memcpy(cbc_chain_data, zero_key, AES_KEY_LENGTH);
    u32 i = 0;
...
int aes_cbc_crypt(char* payload, char* dst, const int enc, uint8_t* key, int len) {
    ...
    if (enc) {
        aes_cbc_encrypt_blocks(key_exp, data, (uint8_t*)dst, aes_blocks);
    } else {
        aes_cbc_decrypt_blocks(key_exp, data, (uint8_t*)dst, aes_blocks);
    }
